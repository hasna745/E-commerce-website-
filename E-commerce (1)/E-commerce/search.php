...
// Check if keywords are provided in the URL
if(isset($_POST['keywords'])) {
    // Retrieve keywords from the URL
    $keywords = mysqli_real_escape_string($con, $_POST['keywords']);

    // Perform a search in the database
    $search_products_query = "SELECT * FROM `product_table` WHERE `procduct_keyw` LIKE '%$keywords%'";
    $search_products_result = mysqli_query($con, $search_products_query);

    if(mysqli_num_rows($search_products_result) > 0) {
        echo "<br>";
        ...
        echo "<br>";
        echo "<br>";
        echo "<p style='color: black;'>No products found matching your search!</p>";
        echo "</div>";
